...
国家は利益と費用を比較して行動する（ということにしておく）。

- 選択肢1の利益 $-$ 費用 $>$ 選択肢2の利益 $-$ 費用 $\leadsto$ 選択肢1を2より好む。
...
- 例：$q = 0.2, x = 0.8, w = 0.5, c_A = c_B = 0.4$
...
- 戦争で有利な結果を得られる国力（$w$は国家Aの軍事的強さとみなせる）
- 現状変更勢力の不満の度合い（$q$は国家Aの現状への満足度とみなせる）
- 戦争で受ける被害・受忍限度（$c_A,c_B$は国家AやBの軍事的強さや決意の固さとみなせる）

![抑止の条件](figures/deterrence_condition.drawio.png)
...
政治的指導者の利益／社会（一般市民）の被害

- 政治生命、旗下結集効果 (rally 'round the flag effect) 、陽動戦争 (diversionary war)
- 天然資源、紛争鉱物
...
干ばつ$\leadsto$農作物の減少・経済水準の低下$\leadsto$戦争の費用の低下
...
**情報の非対称性** (asymmetric information) 、私的情報 (private information)
...
![[アチソン・ライン]()](https://www.sankei.com/resizer/v2/HEPMBA2R65I4ZPF3ANDEWRSKXQ.jpg?auth=ac3eb5593e45af7599d3973d434d2e457f004cf30f975ad8b4cb0882bbb8d408)
...
## 情報伝達のベイズ的定式化
相手国のタイプ$a$が1であるか、0であるかを知りたい。
大国からのメッセージを$m$とする。

$$
P(a=1 \mid m) = \frac{P(m \mid a=1) P(a=1)}{P(m \mid a=1) P(a=1) + P(m \mid a=0) P(a=0)}
$$

$P(a =  1 \mid m) > P(a = 1)$となるのはどのようなときか？

:::

#### 誤認

情報処理における人間の心理的傾向（バイアス）

- 利用可能性 (availability) バイアス、確証 (confirmation) バイアス、動機づけられた推論 (motivated reasoning)
- 自己の誤認：自信過剰、ナイーブ・リアリズム、第三者効果
- 他者の誤認：誤った投影、誤った解釈、帰属 (attribution) バイアス
- 集団と誤認：エコー・チェンバー、同調圧力、忖度

### コミットメント問題

**コミットメント問題** (commitment problem)

- 「国家$B$は約束（コミットメント）を守らないに違いない」と国家Aが考える。
...
- 互いに攻撃する戦争の場合$w$
- 国家$A$が一方的に攻撃する場合$w^A$
- 国家$B$が一方的に攻撃する場合$w^B$
...
大量破壊兵器（例えば核兵器）や戦略的価値のある領土をめぐる交渉
